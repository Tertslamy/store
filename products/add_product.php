...
// Security check for the map embed
$sanitized_map_embed = '';
if (!empty($product['map_embed'])) {
    $pattern = '/^<iframe src="https:\/\/www\.google\.com\/maps\/embed\?pb=.+?" width="\d+" height="\d+" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"><\/iframe>$/';
    if (preg_match($pattern, $product['map_embed'])) {
        $sanitized_map_embed = $product['map_embed'];
    }
}
?>
    ...
    <div class="container">
        ...
        <div class="product-content">
            <div class="product-images">
                <img id="mainImage" class="main-image" src="<?php echo htmlspecialchars($images[0]['image_path'] ?? ''); ?>" alt="<?php echo htmlspecialchars($product['name']); ?>">
                    ...
                    <?php
                        ...
                        switch (strtolower($product['condition'])) {
                            ...
                            case 'new':
                                ...
                                $condition_label = 'ថ្មី';
                                break;
                            case 'មួយទឹក':
                            case 'used':
                                $condition_class = 'condition-used';
                                $condition_label = 'មួយទឹក';
                                break;
                            default:
                                $condition_class = 'condition-refurbished';
                                $condition_label = htmlspecialchars($product['condition']);
                                break;
                        }
                    ?>
                    <div class="condition-badge <?php echo $condition_class; ?>">
                        <?php echo $condition_label; ?>
                    </div>
                <?php endif; ?>
        ...
        <?php if (!empty($product['map_embed'])): ?>
            <div class="map-section">
                ...
                <?php if (!empty($sanitized_map_embed)): ?>
                    <div class="map-embed">
                        <?php echo $sanitized_map_embed; ?>
                    </div>
                <?php else: ?>
                    <div class="location-text" style="text-align: center; color: red;">
                        Google Map embed URL is invalid or insecure.
                    </div>
                <?php endif; ?>
